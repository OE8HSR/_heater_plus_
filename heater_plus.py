...
class SensorConfig:
    ...
    tcs_addr: int
    # TCS3448: Integration (atime 0..255, astep 0..65535), Gain (0..12, see datasheet)
    tcs_atime_steps: int
    tcs_astep: int
    tcs_gain_index: int
...
class LogicConfig:
    ...
    tsl_saturation: int
    filename_json: str
    enable_json: bool   # Create/update JSON file (e.g. for Allsky overlay)
    tmp_settings_path: str
    sensor_only: bool
    # Autogain: Start at minimal gain, increase when signal too low, decrease at saturation
    tsl_autogain: bool
    tcs_autogain: bool
    tsl_high_threshold: int   # Below: OK; above: reduce gain/integration
    tsl_low_threshold: int    # Below: increase gain/integration
    tcs_high_threshold: int   # Channel value above → reduce gain
    tcs_low_threshold: int    # Channel value below → increase gain
    # Safety: dome temp limit; heater power check
    temp_max_dome: float       # Heater off when dome >= this (°C), 0 = disabled
    heater_power_check: bool   # Check that current rises when heater on
    heater_duty_min_for_check: float   # Duty (%) above which to run check
    heater_min_current_above_idle: float  # Expected current rise (mA) when heater on
    heater_fault_cycles: int   # Cycles with too low current → heater fault
    # Log file (empty = console only)
    log_file: str
    # Sensor fault: heater off when BME dome yields no valid temp for X seconds (0=disabled)
    sensor_fault_timeout_sec: float
    # Graceful shutdown: ramp heater down over X seconds instead of instant off (0=instant)
    shutdown_heater_ramp_sec: float
    # PID: Proximity band (°C) – reduce heater when closer than this to setpoint (0=off)
...
class HeaterPlusController:
    def __init__(self, settings_path="/home/pi/heater_plus/hp_settings.json"):
        ...
        self.tcs = None
        self.tcs_gain_index = 0  # Runtime for TCS autogain
        self.tcs_atime_steps = 29  # Runtime integration (set in setup_hardware from settings)
        self.tcs_astep = 599
        self.irq = None
        ...
        self.tcs_max_average = deque(maxlen=15)  # TCS autogain: more samples for stable decision
        self.tcs_autogain_cooldown = 0  # Cycles until next change allowed
        ...
        # Dynamic settings reload (check file modification time)
        self._settings_mtime = 0.0
        # Safety: Heater fault (power does not rise when heater driven)
        self._heater_fault = False
        self._heater_low_current_cycles = 0
        self._heater_ok_cycles = 0  # Cycles with Duty=0 to reset fault
        self._log_file = None
        # Sensor fault: timestamp of last valid BME dome reading
        self._last_valid_bme_dome_time = None
        # Graceful shutdown: Prevent double execution of exit_script
    ...
    def load_settings(self):
        ...
        self.logic_cfg = LogicConfig(
            ...
            tcs_high_threshold=int(s.get("tcs_high_threshold", 0)),  # 0 = Auto (85% of max_count)
        ...
        # TSL2591: Start gain and integration from settings (0 = LOW/100ms, auto-increased at saturation)
    ...
    def validate_settings(self, exit_on_error=True):
        """Check sensible limits. If exit_on_error=True: exit with error message. Otherwise return False."""
        err = []
        c = self.logic_cfg
        s = self.sensor_cfg
        p = self.pid_cfg
        ...
        if c.temp_set < -20 or c.temp_set > 80:
            err.append(f"temp_set={c.temp_set} out of range -20..80 °C")
        if c.temp_max_dome > 0 and (c.temp_max_dome < 30 or c.temp_max_dome > 90):
            err.append(f"temp_max_dome={c.temp_max_dome} out of range 30..90 °C (0=off)")
        if p.P < 0 or p.I < 0 or p.D < 0:
            err.append(f"PID values must be >= 0 (pid_p={p.P}, pid_i={p.I}, pid_d={p.D})")
        pb = getattr(c, "pid_proximity_band", 0)
        if pb < 0 or pb > 2.0:
            err.append(f"pid_proximity_band={pb} must be 0..2 °C (0=off)")
        if c.upd_interval_sensor < 1:
            err.append(f"sensorupdateintervall={c.upd_interval_sensor} must be >= 1")
        if c.upd_interval_db < 1:
            err.append(f"updateinterval_db={c.upd_interval_db} must be >= 1")
        for name, addr in [("bme280_dome", s.bme_dome_addr), ("bme280_housing", s.bme_housing_addr),
            ...
            if addr and (addr < 0x08 or addr > 0x77):
                err.append(f"I2C address {name}=0x{addr:02X} out of range 0x08..0x77")
        if s.ina_shunt_ohms <= 0 or s.ina_shunt_ohms > 1:
            err.append(f"ina226_shunt_ohms={s.ina_shunt_ohms} should be 0.001..1 Ohm")
        if g.heater_pin and (g.heater_pin < 2 or g.heater_pin > 27):
            err.append(f"heaterpin={g.heater_pin} out of range 2..27 (BCM), 0=off")
        if g.fan_pin and (g.fan_pin < 2 or g.fan_pin > 27):
            err.append(f"fanpin={g.fan_pin} out of range 2..27 (BCM), 0=off")
        if c.heater_power_check and (c.heater_duty_min_for_check < 0 or c.heater_duty_min_for_check > 100):
            err.append("heater_duty_min_for_check must be 0..100")
        if c.heater_power_check and c.heater_fault_cycles < 1:
            err.append("heater_fault_cycles must be >= 1")
        if c.tsl_high_threshold < 0 or c.tsl_high_threshold > 65535:
            err.append(f"tsl_high_threshold={c.tsl_high_threshold} must be 0..65535")
        if c.tcs_high_threshold < 0 or c.tcs_high_threshold > 65535:
            err.append(f"tcs_high_threshold={c.tcs_high_threshold} must be 0..65535")
        if getattr(c, "sensor_fault_timeout_sec", 10) < 0:
            err.append("sensor_fault_timeout_sec must be >= 0 (0=disabled)")
        if getattr(c, "shutdown_heater_ramp_sec", 1.5) < 0:
            err.append("shutdown_heater_ramp_sec must be >= 0 (0=instant off)")
        if err:
            print("[Settings] Validation failed:")
            for e in err:
                print("  -", e)
            print("Please adjust hp_settings.json.")
            if exit_on_error:
                sys.exit(1)
            return False
        return True
    ...
    def reload_runtime_settings(self, s):
        """Load runtime-relevant settings from dict s (no hardware change)."""
        self.logic_cfg = LogicConfig(
            run=bool(s.get("running", True)),
            debug=bool(s.get("debugoutput", True)),
            send_to_db=bool(s.get("sendtodb", True)),
        ...
        self.log("[Settings] Reloaded (temp_set=", self.logic_cfg.temp_set, ", pid=", self.pid_cfg.P, "/", self.pid_cfg.I, "/", self.pid_cfg.D, ")")

    def check_and_reload_settings(self):
        """Check if hp_settings.json was modified and reload if so."""
        try:
        ...
        try:
            ...
            self.log("[Settings] Reload failed:", e)
            return
        self._settings_mtime = mtime
        if not self.validate_settings(exit_on_error=False):
            self.log("[Settings] Reload skipped: validation failed.")
            return
    ...
    def setup_hardware(self):
        ...
        if self.sensor_cfg.as3935_addr:
            try:
                ...
                self.as3935.set_mask_disturber(False)  # Allow disturber for testing with phone etc.
        ...
        if not self.logic_cfg.sensor_only:
            ...
            self.log("[Init] GPIO PWM skipped (sensor_only).")
    ...
    def influxdb_connect(self):
        if not self.logic_cfg.send_to_db or not INFLUX_AVAILABLE:
            return
        try:
    ...
    def ascii_bar(value, vmin, vmax, width=10):
        """Generate ASCII bar from Unicode blocks. value between vmin and vmax."""
        try:
            v = float(value)
            if vmax <= vmin:
                return "\u2588" * width  # full when undefined
            r = (v - vmin) / (vmax - vmin)
            r = max(0.0, min(1.0, r))
    ...
    def json_write(self):
        """Write sensor values as JSON for Allsky overlay (extra data file)."""
        try:
            ...
            # Format for Allsky allsky_overlay.py (variable → value + optional expires)
            expiry = 60
            out = {
                # BME Dome
            ...
            try:
                ...
                self.log("[JSON] Keine Schreibrechte für Verzeichnis:", p.parent,
                         "- prüfe: ls -la", str(p.parent), "| ggf. sudo chown pi:pi -R")
                return
            try:
                with open(p, "w") as f:
                ...
                self.log("[JSON] Keine Schreibrechte für", p,
                         "- prüfe: sudo chown pi:pi", str(p))
        except Exception as e:
    ...
    def read_tsl(self):
        try:
            ...
            # Autogain: increase when signal too low, decrease at saturation
            if self.logic_cfg and getattr(self.logic_cfg, "tsl_autogain", True):
                ...
                # Saturation: full_spectrum or Lux too high → reduce gain/integration
    ...
    def read_ina(self):
        ...
        try:
            ...
            # Power (mW) = V * I. INA226 measures total current (heater + fan + board), not just heater.
    ...
    def read_tcs(self):
        ...
        try:
            ...
            if self.logic_cfg and getattr(self.logic_cfg, "tcs_autogain", True):
                ...
                asat_reduce = False
                if asat:
                    # ASAT can be triggered by FD (Flicker) even when spectral channels are OK.
                ...
                if not asat_reduce:
                    # Spectral channels only (exclude FD)
                    spectral_keys = [k for k in ch if "fd" not in k.lower()]
    ...
    def read_as3935_poll(self):
        ...
        try:
            ...
            # 0x08 = Lightning, 0x04 = Disturber (for testing with phone etc.)
    ...
    def print_sensor_data(self):
        ...
        # INA226 (current/power = total supply: heater + fan + board, not just heater)
        ...
        # TCS3448 (Gain, optionally Atime, then all channels)
        if self.tcs:
    ...
    def apply_pid(self):
        ...
        # BME Dome missing/disabled: heater off (dangerous without temp feedback otherwise)
        if (
            ...
            self.log("[Safety] Heater off: BME Dome not connected or disabled (bme280_dome_address=0).")

        # Sensor fault: heater off when BME dome yields no valid temp for > X seconds
        c = self.logic_cfg
        if (
            not self.logic_cfg.sensor_only
            ...
            if self._last_valid_bme_dome_time is None:
                self._last_valid_bme_dome_time = now  # First start: no reading yet
            if now - self._last_valid_bme_dome_time > c.sensor_fault_timeout_sec:
                self.heater_duty = 0.0
                self.data["heater_safety_sensor"] = True
                self.log(
                    "[Safety] Heater off: BME Dome no valid temp for",
                    round(now - self._last_valid_bme_dome_time, 1),
                    "s (sensor_fault_timeout_sec=",
        ...
        if not self.data.get("heater_safety_sensor"):
            if not self.pid or not self.logic_cfg.enable_dewheater:
                ...
                # Proximity band: reduce heater when close to setpoint, reduces overshoot
        ...
        # Safety: dome temp limit (prevent overheating)
        c = self.logic_cfg
        if c.temp_max_dome > 0:
            ...
            if t_dome is not None and t_dome >= c.temp_max_dome:
                ...
                self.log("[Safety] Heater off: dome temp", t_dome, ">= temp_max_dome", c.temp_max_dome)

        # Safety: heater power check (current must rise when heater on)
                ...
                if self.heater_duty >= c.heater_duty_min_for_check:
                    if above_idle < c.heater_min_current_above_idle:
                        ...
                        if self._heater_low_current_cycles >= c.heater_fault_cycles:
                            if not self._heater_fault:
                                ...
                                self.log("[Safety] Heater fault: power does not rise at Duty", self.heater_duty, "%; current", round(current, 1), "mA (expected >", round(idle + c.heater_min_current_above_idle, 1), "mA)")
                            self.heater_duty = 0.0
                    else:
                        self._heater_low_current_cycles = 0
                    ...
                    if self._heater_fault:
                        ...
                        if self._heater_ok_cycles >= 5:
                            self._heater_fault = False
                            self._heater_ok_cycles = 0
                            self.log("[Safety] Heater fault reset (Duty was 0)")
        if self._heater_fault:
            self.heater_duty = 0.0
    ...
    def update_fan(self):
        if not self.logic_cfg.enable_fan:
            ...
            # Only evaluate when both sensors have valid values (not None, plausible 0..60°C)
            if td is not None and th is not None and 0 < td < 80 and 0 < th < 80:
                ...
                self.fan_duty = 0.0  # On missing/implausible values: fan off
    ...
    def loop(self):
        ...
        while self.running and self.logic_cfg.run:
            ...
            if self.logic_cfg.sensor_only:
                # Sensors only: output then wait (kein Heater/Fan)
                self.print_sensor_data()
                if time.time() - last_db >= self.logic_cfg.upd_interval_db:
                    if self.logic_cfg.send_to_db:
                        self.influxdb_write()
                    if self.logic_cfg.enable_json:
                        self.json_write()
                    last_db = time.time()
            else:
    ...
    def exit_script(self, *args):
        ...
        # Graceful shutdown: ramp heater down (less voltage spikes, gentler)
        ...
        if ramp_sec > 0 and self.heater and self.heater_duty > 0:
            ...
            self.log("[Exit] Heater ramped down (", round(ramp_sec, 1), "s)")
        if self.heater:
            self.heater.value = 0.0
        if self.fan:
            self.fan.value = 0.0
...
if __name__ == "__main__":
    ...
    if ctrl.logic_cfg.log_file:
        try:
            ...
            print("[Log] Could not open log file:", ctrl.logic_cfg.log_file, e)
    ctrl.setup_hardware()
    # Optional: brief fan test (hp_settings.json: "fan_test_sec": 3)
    with open(ctrl.settings_path) as f:
        _s = json.load(f)
    fan_test = float(_s.get("fan_test_sec", 0))
    if fan_test > 0 and ctrl.fan:
        print(f"[Init] Fan test {fan_test}s (100%)...")
        ctrl.fan.value = 1.0
        time.sleep(fan_test)
        ctrl.fan.value = 0.0
        print("[Init] Fan off.")
